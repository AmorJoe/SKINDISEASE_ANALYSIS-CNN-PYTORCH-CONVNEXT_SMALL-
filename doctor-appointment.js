...
const MEDIA_BASE_URL = API_BASE_URL_DA.replace('/api', '');
    ...
    try {
        ...
        if (result.status === 'success' && result.data.length > 0) {
            container.innerHTML = result.data.map(doc => {
                // Build full URL: relative paths need the backend origin
                const fullFileUrl = doc.file_url && doc.file_url.startsWith('http') ? doc.file_url : `${MEDIA_BASE_URL}${doc.file_url}`;
                return `
                <div id="doc-card-${doc.id}" style="display: flex; align-items: flex-start; gap: 16px; padding: 20px; border-bottom: 1px solid #f0f0f0; transition: background 0.2s;">
                    <div style="width: 44px; height: 44px; border-radius: 10px; background: linear-gradient(135deg, #fee2e2, #fecaca); display: flex; align-items: center; justify-content: center; color: #dc2626; flex-shrink: 0;">
                        <i class="fas fa-file-pdf" style="font-size: 1.1rem;"></i>
                    </div>
                    <div style="flex: 1;">
                        <h4 style="margin: 0 0 4px; color: var(--text-dark); font-size: 0.95rem; font-weight: 600;">${doc.name}</h4>
                        <p style="margin: 0 0 4px; color: #6b7280; font-size: 0.85rem;">
                            <i class="fas fa-user-md" style="margin-right: 4px; font-size: 0.8rem;"></i> ${doc.doctor_name}
                            <span style="margin: 0 6px; color: #d1d5db;">•</span>
                            <i class="fas fa-calendar-alt" style="margin-right: 4px; font-size: 0.8rem;"></i> ${doc.created_at}
                        </p>
                        ${doc.note ? `<p style="margin: 0 0 10px; color: #4b5563; font-size: 0.9rem; background: #f9fafb; padding: 10px 12px; border-radius: 8px; white-space: pre-wrap; border-left: 3px solid var(--primary-color);">${doc.note}</p>` : ''}
                        <div style="display: flex; gap: 8px; margin-top: 8px;">
                            <button type="button" onclick="downloadDocument('${fullFileUrl}', '${doc.name.replace(/'/g, "\\'")}')" style="padding: 7px 14px; font-size: 0.85rem; border-radius: 8px; border: none; background: var(--primary-color); color: white; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; font-family: 'Poppins', sans-serif; font-weight: 500; transition: all 0.2s; box-shadow: 0 2px 6px rgba(43,125,163,0.25);">
                                <i class="fas fa-download"></i> Download PDF
                            </button>
                            <a href="${fullFileUrl}" target="_blank" style="text-decoration: none; padding: 7px 14px; font-size: 0.85rem; border-radius: 8px; border: 1px solid #e5e7eb; color: var(--text-dark); display: inline-flex; align-items: center; gap: 6px; font-family: 'Poppins', sans-serif; font-weight: 500; transition: all 0.2s; background: white;">
                                <i class="fas fa-eye"></i> View
                            </a>
                            <button type="button" onclick="deleteDocument(event, ${doc.id}, '${doc.name.replace(/'/g, "\\'")}')" style="padding: 7px 14px; font-size: 0.85rem; border-radius: 8px; border: 1px solid #fecaca; background: #fff5f5; color: #dc2626; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; font-family: 'Poppins', sans-serif; font-weight: 500; transition: all 0.2s;">
...
async function deleteDocument(event, docId, docName) {
    if (event) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL_DA}/predict/documents?id=${docId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${getAuthToken()}` }
        });
        const result = await response.json();
...
// Force-download a document as PDF
async function downloadDocument(fileUrl, docName) {
    try {
